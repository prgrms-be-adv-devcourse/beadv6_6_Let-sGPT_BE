...
--
-- 공정성 방어(필수): admit.lua의 스캔 범위(맨 앞 maxScan명) 밖에 있는 사용자는 절대 여기서
-- 입장시키지 않는다. 이 검사가 없으면 대기열 뒤쪽(예: 900번째)의 사용자가 자기 순서를
-- 건너뛰고 그 순간의 가용 재고를 가로챌 수 있다 - "순번대로, 못 든 사람은 대기"라는 대전제를
-- 깨는 결함이다. 애플리케이션 계층(QueueService.resolveStatus)도 같은 조건으로 앞쪽이 아니면
-- DECISION_REQUIRED 자체를 안 보여주지만, 그 체크와 이 스크립트 실행 사이의 레이스(막 스캔
-- 범위에 든 순간 등)까지 막는 건 원자적으로 실행되는 이 스크립트뿐이다.
--
...
--
-- ARGV[1]=dropId  ARGV[2]=userId  ARGV[3]=입장권 TTL(초)  ARGV[4]=now(epoch ms)  ARGV[5]=maxScan
--
-- 반환: 실제 발급 수량(정수, > 0) - 0이면 그 순간 가용 재고가 없거나(또는 스캔 범위 밖이라)
-- 아무 것도 안 함.

local userId = ARGV[2]

local rank = redis.call('ZRANK', KEYS[1], userId)
if rank == false or rank >= tonumber(ARGV[5]) then
  return 0
end
