...
5. 결제 미회신 시간초과 → **주문(order)이 타임아웃 감지** → 상품 재고 롤백(보상) API 호출 → `order.failed`. (상품은 롤백 API만 제공하고 타임아웃을 감지하지 않음 — 재고 통신은 동기 API 단일 경로)
...
- 결제 이벤트 미회신 시 정리: **주문(order)이 타임아웃을 감지해 상품 재고 롤백 API를 호출**하는 방식으로 합의(상품은 롤백 API만 제공). 타임아웃 기준 시간은 주문 도메인이 정의.
