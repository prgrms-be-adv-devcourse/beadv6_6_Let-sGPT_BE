...
| 모델 지정 | 별칭만 사용: `chat`, `vision`, `embed` |
| 임베딩 차원 | **1024차원 고정** |
| 스트리밍 | SSE, 요청에 `"stream": true` 지정 |
| 스트림 정상 종료 | 마지막 이벤트 `data: [DONE]` 필수 |
| 요청 본문 상한 | **20MiB** (`20 × 1024 × 1024`바이트) |
...
클라이언트는 실제 provider 모델명을 알 필요가 없으며, 요청에 직접 지정해서도 안 된다. `model` 필드에는 서버가 정의한 별칭만 사용한다.
...
> **중요:** 응답의 `model` 값을 비즈니스 로직이나 분기 조건으로 사용하지 않는다. 응답의 `model`은 실제 요청을 처리한 provider 모델명이며, 서버의 모델 교체나 폴백에 따라 달라질 수 있다.
...
요청 본문 상한 20MiB는 원본 이미지 파일의 크기가 아니라 **base64 문자열과 JSON 구조를 모두 포함한 최종 HTTP 본문 크기**에 적용된다. base64 인코딩 결과는 일반적으로 원본 바이너리보다 약 33% 커지므로, 큰 이미지는 전송 전에 리사이즈하거나 압축한다.

예를 들어 원본 이미지가 15MiB에 가깝다면 base64 인코딩만으로도 20MiB를 초과할 수 있으며, JSON 구조까지 포함하면 요청이 `413`으로 거절될 가능성이 높다.
...
위 예시는 표현 형식만 보여 주기 위해 벡터 값을 축약했다. 실제 `embedding` 배열에는 항상 **1024개의 숫자**가 들어간다.
...
3. 결과 원소 수가 1024인지 검증한다.

### 8.4 1024차원 고정 계약

임베딩 차원과 벡터 공간은 기존 색인과의 호환성을 보장하기 위한 고정 계약이다. 클라이언트가 임의로 차원을 변경하거나 다른 임베딩 모델의 결과로 대체해서는 안 된다.

임베딩 모델이 달라지면 차원이 같더라도 벡터 공간이 다를 수 있다. 서로 다른 모델의 벡터를 같은 색인에 섞으면 기존 벡터와 새 벡터 사이의 유사도 비교가 의미를 잃는다.

따라서 다음 원칙을 지킨다.

- 검색 색인의 벡터 차원은 항상 1024로 설정한다.
- `embed` 요청이 실패하면 재시도한다.
...
| `413` | 최종 요청 본문이 20MiB 초과 | 입력을 줄인다. 이미지는 리사이즈·압축한다 |
...
`chat`과 `vision`이 폴백되면 요청은 성공할 수 있지만 응답의 `model` 값은 평소와 달라질 수 있다. 이는 정상적인 동작이다. `model` 값은 관측이나 진단 목적으로 기록할 수 있지만, 서비스 동작을 결정하는 분기 조건으로 사용하지 않는다.

`embed`가 폴백하지 않는 이유는 벡터 공간의 일관성을 지키기 위해서다. 다른 임베딩 모델의 결과를 기존 색인에 섞으면 차원이 같더라도 검색 품질과 유사도 비교의 의미가 깨질 수 있다.

따라서 색인 파이프라인은 `embed` 실패를 다음과 같이 처리한다.

```text
embed 실패 → 재시도 또는 작업 보류
embed 실패 → 다른 임베딩 모델로 대체 금지
```
...
- `dense_vector` 매핑의 `dims`는 **1024**로 설정한다.
- `embed` 실패는 재시도 가능한 작업 실패로 처리한다.
- 실패한 임베딩을 다른 모델의 벡터로 대체하지 않는다.
- 다건 색인은 여러 문장을 `input` 배열 한 번에 보내 왕복 횟수를 줄인다.
- 응답의 `data[].index`를 기준으로 원문과 임베딩을 연결한다.
- 배치 요청의 최종 JSON 본문이 20MiB를 넘지 않도록 배치 크기를 제한한다.
...
`embed` 요청을 한 번 실행하고 다음 항목을 검사한다.

- 응답 벡터 길이가 1024인지 확인한다.
- 배치 요청에서 `data[].index`가 입력과 올바르게 연결되는지 확인한다.
- 검색 색인의 `dense_vector.dims`가 1024인지 확인한다.
...
| 임베딩 요청만 실패함 | `embed`는 폴백하지 않으므로 재시도 또는 작업 보류 |
...
- [ ] `model`에는 `chat`, `vision`, `embed` 별칭만 사용한다.
...
- [ ] base64와 JSON을 포함한 최종 본문이 20MiB 이하인지 확인한다.
- [ ] 큰 이미지는 전송 전에 리사이즈하거나 압축한다.

### 검색·임베딩

- [ ] Elasticsearch `dense_vector.dims`가 1024다.
- [ ] 실제 응답 벡터 길이를 1024로 검증한다.
- [ ] 배치 응답을 `data[].index`로 원문과 연결한다.
- [ ] `embed` 실패를 다른 임베딩 모델로 대체하지 않는다.
- [ ] `embed` 실패는 재시도 또는 작업 보류로 처리한다.
- [ ] 배치 요청이 20MiB를 넘지 않도록 크기를 제한한다.
