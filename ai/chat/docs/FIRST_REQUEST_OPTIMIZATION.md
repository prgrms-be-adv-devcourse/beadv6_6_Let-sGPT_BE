...
임의 재귀나 무한 도구 루프는 사용하지 않는다. 일반 질문은 1개, 가벼운 도구 질문은 2개, 상세 내부 데이터 질문은 최대 3개의 고정 논리 단계로 처리한다. 단계 안의 schema shard와 빈 route·깨진 binding 형식의 각 1회 복구는 bounded 물리 호출이며 논리 깊이를 늘리지 않는다.
...
- 초기 도구 wire 정의와 공통 시스템 지시는 현재 회귀 검사 기준 5,447자로 상한 6,000자 안이다.
