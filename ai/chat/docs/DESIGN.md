...
3. **실행 깊이는 고정한다.** 임의 재귀나 무한 tool loop 없이 질문 유형에 따라 최대 3개의 논리 단계로 끝낸다. 같은 단계의 제한된 빈 응답·형식 복구와 schema shard는 단계 깊이를 늘리지 않는다.
...
질문 유형별 순차 논리 단계는 다음과 같다. 8K 예산으로 같은 단계를 병렬 분할하더라도 순차 깊이는 늘지 않는다.

| 질문 유형 | 논리 단계 | 흐름 |
|---|---:|---|
| 일반 지식 | 1개 | 1차 응답 완료·모드 검증 후 자연어 답변 |
| 가벼운 도구만 필요 | 2개 | 선택 → 실행 → 자연어 답변 |
| 내부 데이터만 필요 | 최대 3개 | 영역 선택 → 구조화 → 조회 결과 답변 |
| 가벼운 도구와 내부 데이터 복합 | 최대 3개 | 동시 선택 → 먼저 완료된 결과 답변·구조화 → 최종 답변 |

여러 도구가 필요해도 도구 하나마다 LLM을 다시 호출하지 않는다. 같은 단계의 도구와 조회는 병렬 실행한다.
물리 LLM 호출 수는 입력 예산에 따른 2차 shard 분할과 빈 route·깨진 binding 형식의 각 1회 복구 때문에 논리 단계 수보다 많을 수 있다. shard 수와 복구 횟수는 설정·코드 상한으로 제한하며 성공한 도구나 조회를 다시 실행하는 무제한 재시도는 하지 않는다.
...
1차 응답은 끝까지 수집해 `content only`와 `tool_calls` 모드를 먼저 확정한다. 도구 호출이 없고 `finishReason=stop`인 경우에만 검증한 `content`를 SSE 자연어 조각으로 전달한다. `length`처럼 완결되지 않은 종료 이유는 답변을 노출하지 않고 선택 실패로 처리한다. tool call이 하나라도 있으면 같은 응답의 `content`는 사용자에게 노출하지 않고, 필요한 일반 설명은 이후 근거 답변 단계에서 다시 생성한다. 이 규칙으로 스트림 중 뒤늦게 도구 호출이 나타나 내부 처리 문장이 먼저 노출되는 경우를 막는다.
...
가벼운 도구 결과는 내부 데이터 구조화 요청에 함께 넣는다. 2차 LLM은 상세 조회를 준비하는 동안 이미 확인된 독립 사실을 먼저 자연어로 답할 수 있다. 이때 실제로 설명한 `SUCCESS`·`PARTIAL` 근거 ID를 함께 반환하고, 서버는 존재하는 ID만 검증해 전달 완료로 표시한다. 선택 실패를 포함한 `FAILED` 근거는 먼저 전달한 것으로 간주하지 않는다.
...
- 같은 단계의 외부 도구와 내부 조회는 하나의 stage timeout 안에서 병렬 실행하고, 완료된 형제 결과는 다른 작업의 실패·timeout과 관계없이 보존한다. 최종 답변 스트림도 청크 간 유휴 시간이 아니라 단계 시작 기준의 절대 stage timeout을 적용하고, 만료 시 upstream을 취소한다. 도구 하나가 끝날 때마다 LLM을 추가 호출하지 않는다.
- 모든 route·binding·최종 답변은 실제 모델 호출 직전에 system·user·tool 정의를 함께 계산한 입력 예산 검사를 통과해야 한다.
- 직전 대화 때문에 예산을 넘으면 선택 정보인 직전 한 턴만 제외하고 현재 질문을 다시 검사한다. 현재 질문과 필수 근거도 예산을 넘을 때만 요청을 거부한다.
...
로컬 모델에는 `reasoning_effort=none`, 온도 0, 자동 재시도 0, `store=false`를 적용했다. 8K 환경에서 1차 초기 도구 wire 정의와 공통 시스템 지시는 현재 회귀 검사 기준 5,447자이고, 여섯 상세 영역 조합은 약 3,583 tokens로 입력 목표 6,000 tokens 안에 들어와 현재는 한 shard에서 처리된다. 요청 전 검사는 현재 애플리케이션이 제공하는 근사 토큰 추정기와 692-token 안전 여유를 사용하며, Gemma 서버의 실제 tokenizer와 같다고 가정하지 않는다. 예산을 넘는 향후 상세 스키마 확장은 설정된 최대 shard 수 안에서 같은 2차 단계로 분할하고, 호출 직전에도 상한을 다시 확인한다.
