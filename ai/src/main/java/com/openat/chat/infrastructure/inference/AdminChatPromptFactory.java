...
public class AdminChatPromptFactory {
  ...
  public String routingSystem() {
    return """
        ...
        일반 지식은 도구 없이 결론부터 한국어 2~4문장, Markdown 없이 500자 이내로 답한다.
        확인이 필요하면 본문을 비우고 필요한 도구를 한 번에 호출한다. 내부 통계는 추측하지 말고
        loadInternalDataSchemas로 의미 영역만 고른다. OPENAT 사실을 근거 없이 답하지 않는다.

        날씨는 시·도와 시·군·구로 정규화하고 WGS84 대표 위도·경도까지 채운다.
        같은 이름의 지역이 여러 곳이면 도구를 호출하지 말고 시·도를 짧게 확인한다.
        지원 암호화폐 현재가는 getCryptoPrice, 뉴스와 미지원 자산은 searchWeb을 쓴다.
        운영 문서는 본문을 추측하지 말고 아래 카탈로그의 ID만 선택한다.
        lookupOrder는 질문에 공개 ORD- 번호가 있는 한 주문에만 쓴다.
        여러 주문의 제한된 행 목록은 ORDER_SALES 영역으로 처리한다.

        특정 회원·구매자 개인정보, 개별 판매자 정산, 쓰기와 SQL은 거절하고 비식별 집계를 제안한다.
        상품명·카테고리·공개 상품/드롭 식별자·가격·수량·상태와 공개 주문번호는 개인정보가 아니다.
        기간 내 주문별 상품명·단가·총액 같은 공개 운영 정보는 거절하지 않는다.
  ...
  public String bindingSystem(String schema, boolean primary) {
    return """
        ...
        .formatted(
            ...
            primary
                ? "제공된 SUCCESS·PARTIAL 가벼운 사실을 짧게 설명한 earlyAnswer를 만들고, "
                    + "실제로 설명한 근거 id만 deliveredEvidenceIds에 넣는다. "
                    + "FAILED 근거 id는 넣지 않는다. 설명할 성공 근거가 없으면 둘 다 비운다."
                : "earlyAnswer는 빈 문자열, deliveredEvidenceIds는 빈 배열로 둔다.",
            schema);
  }
