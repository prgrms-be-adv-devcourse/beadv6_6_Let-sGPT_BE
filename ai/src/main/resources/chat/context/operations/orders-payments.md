...
3. 결제 완료 결과를 받으면 주문을 `COMPLETED`로 확정하고 `order.completed.events`를 발행한다. 결제가 이 이벤트로 판매자·상품 문맥을 보완한 뒤 `payment.settlement.events`를 발행하고, 정산이 이를 적재한다.
4. `payment.failed.events`는 실패 시도 이력만 남기고 주문을 `PAYMENT_PENDING`으로 유지한다.
   결제 기한이 지나면 payment 최종 상태를 먼저 조회하고, 미결제·실패·취소가 확인될 때
   주문을 실패 처리하고 확보한 재고 복원을 시도한다.
...
관리자 내부 분석은 결제 시도·승인·실패·순결제액과 환불 요청·완료·금액을
상태·결제 수단·PG·대사 상태 기준으로 집계할 수 있다. 이는 비식별 집계이며 개인별 결제·환불
행, 결제 키와 자유 실패 원문은 제공하지 않는다.
