 ...
 *
 * <ul>
 *   <li>{@code X-Seller-Id} = {@code sub}(sellerInfoId) — 테넌트 헤더
 *   <li>{@code X-User-Id} / {@code X-User-Roles}는 주입하지 않고 strip ({@code sub}=sellerInfoId가 {@code
 *       X-User-Id}로 오염되는 것 방지)
 *   <li>위임 감사: {@code act.sub}(memberId) + {@code sub}(sellerInfoId)를 INFO 로그
 * </ul>
 *
 * <p>안티-스푸핑: 토큰 유무와 관계없이 클라이언트가 보낸 {@code X-User-Id}/{@code X-User-Roles}/{@code X-Seller-Id}를 항상
 * 제거한다.
 */
@Component
public class UserContextRelayFilter implements GlobalFilter, Ordered {
  ...
  private ServerWebExchange mutateWithUserHeaders(
      ServerWebExchange exchange, JwtAuthenticationToken auth) {
  ...
  /**
   * scoped 토큰 (RFC 8693 delegation): X-Seller-Id(sub=sellerInfoId)만 주입. X-User-Id/X-User-Roles는
   * 주입하지 않음(sub=sellerInfoId가 X-User-Id로 오염 방지). act.sub(memberId) + sub(sellerInfoId)를 감사 로그에 기록.
   */
  @SuppressWarnings("unchecked")
  private ServerWebExchange mutateWithScopedHeaders(
      ServerWebExchange exchange, JwtAuthenticationToken auth) {
