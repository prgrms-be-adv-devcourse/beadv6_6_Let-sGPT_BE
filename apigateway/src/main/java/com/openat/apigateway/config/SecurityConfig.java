...
public class SecurityConfig {
   ...
   *
   * <p>{@code permitAll()} skips authorization only. An expired Bearer token saved by Swagger can
   * otherwise be rejected with 401 before the authorization rule is evaluated.
   */
  @Bean
  @Order(0)
  public SecurityWebFilterChain publicSearchSecurityWebFilterChain(
    ...
    http.securityMatcher(ServerWebExchangeMatchers.pathMatchers("/api/v1/searchs/**"))
   ...
   * "reactiveJwtSecurityFilterChain"이라는 기본 체인을 추가로 자동 등록하는데, 이 체인은 CORS/permitAll 목록 등 우리가 정의한 규칙을
   * 전혀 모른 채 그냥 "인증됐는지"만 본다. {@code SecurityWebFilterChain}이 여러 개일 때 Spring Security는
   * {@code @Order}가 낮은(우선순위 높은) 체인의 매처가 매칭되면 그 체인을 쓰는데, 명시적 순서를 안 주면 우리 체인이 항상 이긴다는 보장이 없어 일부 경로가
   * 자동 체인으로 새서 permitAll이어야 할 경로가 보호되는 등 어긋났다. 항상 우리 체인이 먼저 선택되도록 최우선순위를 명시한다. (라우트 존재 여부 체크는 더 이상
   * 이 체인에 끼워넣지 않는다 — {@link RouteExistenceFilter} 참고.)
   */
  @Bean
  @Order(1)
        ...
        .authorizeExchange(
            exchange ->
                exchange
                    ...
                    // 관리자 챗봇은 게이트웨이와 AI 서비스에서 ADMIN을 각각 확인한다.
                    .pathMatchers("/api/v1/ai/chats", "/api/v1/ai/chats/**")
                    .hasRole("ADMIN")

                    // payment 웹훅 — Toss PG가 JWT 없이 직접 호출 (apigateway/docs/SECURITY_CONFIG_GUIDE.md
                    // 패턴).
                    ...
                    //  2) /payment/... — payment 라우트(Path=/payment/**+StripPrefix=1, compose 프로필
                    // 한정).
                    ...
                    .pathMatchers(
                        ...
                        "/payment/api/v1/refunds/webhook")
                    .permitAll()

                    // payment 내부 운영 경로(DLQ 재처리, PG 대사 실행, 정산 이벤트 테스트, 정산 조회) —
                    // payment 서비스 자체 SecurityConfig가 permitAll 전면 개방이라 게이트웨이 인가가 유일한 방어선.
                    // /payment/** 라우트(StripPrefix=1)로 /payment/internal/... 이 서비스의 /internal/... 로
                    // 전달된다.
                    .pathMatchers("/payment/internal/**")
                    .hasRole("ADMIN")
                    ...
                    .pathMatchers(HttpMethod.POST, "/api/v1/seller/me")
                    .access(authenticatedAndNotScoped())

                    // 본인 판매자 정보 조회·수정·삭제 — 이미 판매자인 경우만
                    // hasRole()은 roles 클레임 없는 scoped 토큰을 이미 거부하므로 안전
                    .pathMatchers(HttpMethod.GET, "/api/v1/seller/me")
                    .hasRole("SELLER")
                    .pathMatchers("/api/v1/seller/me/**")
                    .hasRole("SELLER")

                    // 관리자 전용: userId로 해당 회원의 판매자 정보 전체 조회
                    .pathMatchers(HttpMethod.GET, "/api/v1/seller/*")
                    .hasRole("ADMIN")

                    // 그 외 seller 경로 (확장 대비) — scoped 토큰 명시적 거부
                    .pathMatchers("/api/v1/seller/**")
                    .access(authenticatedAndNotScoped())

                    // 정산 관리자 전용
                    .pathMatchers(HttpMethod.GET, "/api/v1/settlements/admin/*")
                    .hasRole("ADMIN")
                    .pathMatchers(HttpMethod.GET, "/api/v1/settlements/seller/*")
                    .hasRole("SELLER")
                    ...
                    .access(scopedFor("openat-product"))
                    .pathMatchers(
  ...
  /**
   * swagger-ui(localhost:8000)에서 각 서비스로 직접 "Try it out" 호출할 때 발생하는 CORS 문제를 막기 위함. 로컬 개발 전용이라
  ...
  /**
   * scoped 토큰(typ=scoped)을 명시적으로 거부하는 공통 인가 관리자. JWT 인증이 됐더라도 scoped 토큰은 product write 경로 외에서 진입할 수
   * 없어야 한다. 역할 없는 순수 {@code .authenticated()} 경로에서도 scoped 토큰이 새는 것을 방지한다.
   */
  ...
  /**
   * scoped 토큰(typ=scoped)이고 지정 audience를 포함하는 경우만 허용하는 인가 관리자. product write처럼 특정 서비스 전용 scoped 토큰이
   * 필요한 경로에 사용한다.
   */
