...
public class AdmissionCheckGatewayFilterFactory
     ...
     * 주문하므로 흔치 않은 경로).
     *
     * 수량이 맞으면 {@link Mono#usingWhen}으로 주문 응답이 끝나는 시점(성공/실패/취소 전부)에
     * outstanding을 반드시 되돌린다 - order/product 모듈을 건드리지 않고, 게이트웨이가 이미
     * 리버스 프록시로서 보고 있는 응답 완료 시점만으로 "점유 해제"를 감지한다.
     */
        ...
        return redisTemplate.opsForValue().getAndDelete(admissionKey)
                ...
                .flatMap(qtyStr -> {
                    ...
                    return Mono.usingWhen(
                            ...
                            (qty, error) -> releaseOutstanding(dropId, userId, qty),
                            qty -> releaseOutstanding(dropId, userId, qty)
                    );
                });
    }

    /** 주문 요청이 끝난 뒤(성공/실패/취소 무관) 미소진 입장권 추적에서 제거하고 outstanding을 되돌린다. */
