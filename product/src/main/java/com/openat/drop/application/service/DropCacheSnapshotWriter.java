...
public class DropCacheSnapshotWriter {
  ...
  public void write(UUID dropId, UUID recoveryOwner) {
    ...
    // Retain the lifecycle row lock through publication; Redis checks the current recovery owner.
    dropCacheRepository.warm(
        new DropCacheState(
            dropId, remaining, drop.getOpenAt(), drop.getCloseAt(), drop.getLimitPerUser(), buyers),
        recoveryOwner);
  }
}
