...
public interface ProductJpaRepository extends JpaRepository<Product, UUID> {
  ...
  @Lock(LockModeType.PESSIMISTIC_WRITE)
  @Query(
      "select product from Product product where product.sellerId = :sellerId order by product.id")
  List<Product> findAllBySellerIdForUpdate(@Param("sellerId") UUID sellerId);

  @Lock(LockModeType.PESSIMISTIC_WRITE)
  @Query(
      "select product from Product product where product.category.id = :categoryId order by product.id")
  List<Product> findAllByCategoryIdForUpdate(@Param("categoryId") UUID categoryId);
