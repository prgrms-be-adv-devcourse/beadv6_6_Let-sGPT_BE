...
public class OrderEventConsumer {
    ...
    public void onPaymentCompleted(ConsumerRecord<String, String> record) {
        handle(record.value(), "payment.completed.events", PaymentCompleteEvent.class, event -> {
    ...
    public void onPaymentFailed(ConsumerRecord<String, String> record) {
        handle(record.value(), "payment.failed.events", PaymentFailedEvent.class, event -> {
    ...
    public void onRefundCompleted(ConsumerRecord<String, String> record) {
        handle(record.value(), "refund.completed.events", RefundCompletedEvent.class, event -> {
    ...
    public void onRefundFailed(ConsumerRecord<String, String> record) {
        handle(record.value(), "refund.failed.events", RefundFailedEvent.class, event -> {
    ...
    private <T> void handle(String payload, String topicLabel, Class<T> eventType, Consumer<T> action) {
        try {
            action.accept(objectMapper.readValue(payload, eventType));
        } catch (BusinessException e) {
            log.error("{} consume failed. payload={}", topicLabel, payload, e);
            throw e;
        } catch (Exception e) {
            log.error("{} consume failed. payload={}", topicLabel, payload, e);
            throw new RuntimeException(topicLabel + " consume failed", e);
        }
    }
}
