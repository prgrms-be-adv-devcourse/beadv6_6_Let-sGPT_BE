...
public class OutboxEventPublisher {
    ...
    public void publish(UUID outboxEventId) {
        ...
        if (event == null || event.getStatus() != com.openat.order.domain.model.OutboxEventStatus.PENDING) {
            return;
        }
        ...
        try {
            ...
            event.markFailed();
            log.error("Outbox event payload is invalid; marked FAILED. outboxEventId={}, topic={}",
                    event.getId(), event.getTopic(), exception);
        } catch (InterruptedException exception) {
            Thread.currentThread().interrupt();
            log.warn("Outbox event publishing interrupted; remains PENDING. outboxEventId={}, topic={}",
                    event.getId(), event.getTopic(), exception);
        } catch (Exception exception) {
            log.error("Outbox event publish failed; remains PENDING. outboxEventId={}, topic={}",
                    event.getId(), event.getTopic(), exception);
        }
    }
