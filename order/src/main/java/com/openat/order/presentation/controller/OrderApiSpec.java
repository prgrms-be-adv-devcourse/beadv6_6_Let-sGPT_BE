...
public interface OrderApiSpec {
    ...
    @Operation(summary = "주문 생성", description = "주문을 생성하고 결제 대기 상태로 전환한다.")
    @ApiResponse(
            responseCode = "201",
            description = "생성 성공",
    ...
    @Operation(summary = "주문 조회", description = "주문 단건 조회")
    @ApiResponse(responseCode = "200", description = "조회 성공")
    ResponseEntity<OrderResponse> getOrder(@CurrentUser UserContext userContext, UUID orderId);

    @Operation(summary = "내 주문 목록 조회", description = "로그인 사용자의 주문 목록 조회")
    @ApiResponse(responseCode = "200", description = "조회 성공")
    ...
    @Operation(summary = "주문 취소 요청", description = "결제 대기 주문은 CANCELLED, 완료 주문은 CANCEL_REQUESTED로 전환한다.")
    @ApiResponse(responseCode = "200", description = "요청 성공")
    ResponseEntity<OrderCancelResponse> cancelOrder(@CurrentUser UserContext userContext, UUID orderId);

    @Operation(summary = "결제용 주문 조회", description = "결제 도메인이 주문 검증에 사용하는 내부 API")
    @ApiResponse(responseCode = "200", description = "조회 성공")
