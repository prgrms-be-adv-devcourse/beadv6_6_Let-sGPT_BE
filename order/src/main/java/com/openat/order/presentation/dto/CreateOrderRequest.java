...
public record CreateOrderRequest(
    @Schema(description = "주문 대상 드롭 id") @NotNull UUID dropId,
    @Schema(description = "주문 수량", example = "1") @Min(1) int quantity,
    @Schema(description = "주문 생성 멱등키", example = "order-20260626-0001") @NotBlank
        String idempotencyKey,
    @Schema(description = "주문 표시명 폴백. 스냅샷 productName 제공 전까지만 사용") @Size(max = 255)
        String orderName) {
