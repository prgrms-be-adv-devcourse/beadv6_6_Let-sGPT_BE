...
public class PendingOrderCreator {
  ...
  @Transactional
  public Order create(
      UUID memberId, CreateOrderCommand command, OrderSnapshotInfo snapshot, Instant now) {
    ...
    Order order =
        Order.create()
            ...
            .productName(resolveProductName(command, snapshot))
  ...
  private String resolveProductName(CreateOrderCommand command, OrderSnapshotInfo snapshot) {
    if (StringUtils.hasText(snapshot.productName())) {
      return snapshot.productName().trim();
    }
    if (StringUtils.hasText(command.orderName())) {
      return command.orderName().trim();
    }
