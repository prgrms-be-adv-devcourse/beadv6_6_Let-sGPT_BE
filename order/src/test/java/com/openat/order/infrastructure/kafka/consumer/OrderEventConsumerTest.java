...
class OrderEventConsumerTest {
  ...
  @Mock private InboxEventProcessor inboxEventProcessor;

  @Mock private InboxEventFailureRecorder inboxEventFailureRecorder;
  ...
  void onPaymentCompleted_delegatesCommand() {
    ...
    verify(orderEventService)
        .handlePaymentCompleted(new PaymentCompletedCommand(orderId, paymentId, 10_000L));
    verify(inboxEventProcessor)
        .process(
  ...
  void onPaymentFailed_delegatesCommand() {
    ...
    verify(orderEventService)
        .handlePaymentFailed(new PaymentFailedCommand(orderId, paymentId, "PG_TIMEOUT"));
  }
  ...
  void onRefundCompleted_delegatesCommand() {
    ...
    verify(orderEventService)
        .handleRefundCompleted(new RefundCompletedCommand(orderId, paymentId, 10_000L, refundId));
  }
  ...
  void onPaymentCompleted_whenPayloadInvalid_throwRuntimeException() {
    assertThrows(
        ...
        () ->
            orderEventConsumer.onPaymentCompleted(
                record("payment.completed.events", "invalid-json")));
    verify(orderEventService, org.mockito.Mockito.never()).handlePaymentCompleted(any());
    verify(inboxEventFailureRecorder)
        .record(
