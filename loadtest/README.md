...
> ### 시작 전 확인 (2026-07-23 갱신)
> `PG_BASE_URL` 오버라이드(`pg.base-url`)는 **main에 머지 완료** — 배포된 payment 이미지가
> 지원한다. 그래도 아래 4번 절의 WireMock 저널 확인은 매번 수행한다(이미지 롤백 등 어떤
> 이유로든 오버라이드가 안 먹으면 실 토스를 때리게 되므로, 저널이 유일한 실증이다).
>
> **auto-sync 일시정지 내구성 (2026-07-23 확보)**: 예전에는 openat Application의 automated를
> 꺼도 자기감시(argocd) Application이 깃 기준으로 몇 분 안에 되살렸다 — 목이 살아남은 건
> 우연이었다. 지금은 자기감시 앱이 `spec.syncPolicy.automated` 차이를 무시하도록 수정되어
> (`RespectIgnoreDifferences`) 일시정지가 off까지 유지된다. 단 ServerSideApply와의 필드
> 소유권 엣지케이스는 미확증이므로, 목을 켠 동안 `automated` 가 null로 유지되는지
> `kubectl -n argocd get application openat -o jsonpath='{.spec.syncPolicy.automated}'` 로
> 중간에 한 번 확인할 것.
...
curl -s "http://$WM:8080/__admin/requests/count"
...
```bash
WM=$(kubectl -n openat get svc wiremock-toss -o jsonpath='{.spec.clusterIP}')
N=$(curl -s "http://$WM:8080/__admin/requests/count" | grep -o '[0-9]\+' | head -1)
...
- `count` 가 smoke의 성공 건수만큼 올라갔다 → 목이 붙었다. 계속 진행.
- `count` 가 **0이다** → payment는 목을 부르지 않고 있다. 배포된 payment 이미지가
