...
> ### ⚠️ 시작 전 확인 (2026-07-20 현재 미해결)
> `PG_BASE_URL` 오버라이드는 `payment/.../TossClientConfig.java` +
> `payment/src/main/resources/application.yml` 변경에 달려 있는데, 이 변경은
> **아직 `main`에도 `deploy/state`에도 없다**(로컬 미커밋 상태). 배포된 payment 이미지가
> 이 변경을 포함하지 않으면 `PG_BASE_URL` 을 넣어도 **아무 효과가 없고 실 토스페이먼츠를
> 때린다.** 아래 §4의 WireMock 저널 확인을 통과한 뒤에만 램프업할 것.
