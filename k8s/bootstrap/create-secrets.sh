...
export -n DB_USER DB_PASSWORD AI_QUERY_DB_PASSWORD JWT_KEY_ID JWT_PRIVATE_KEY JWT_PUBLIC_KEY \
  ...
  OPENAI_INFERENCEAI_BASE_URL OPENAI_INFERENCEAI_API_KEY \
  OPENAI_INFERENCEAI_EMBEDDING_ENABLED CHAT_INFERENCE_API_KEY TAVILY_API_KEY \
  GHCR_USER GHCR_PAT WARMUP_EMAIL WARMUP_PASSWORD \
...
for key in DB_USER DB_PASSWORD AI_QUERY_DB_PASSWORD JWT_KEY_ID JWT_PRIVATE_KEY JWT_PUBLIC_KEY \
  PG_CLIENT_KEY PG_SECRET_KEY PAYMENT_FIELD_ENCRYPTION_KEY OPENAI_API_KEY \
  OPENAI_INFERENCEAI_BASE_URL OPENAI_INFERENCEAI_API_KEY CHAT_INFERENCE_API_KEY \
  TAVILY_API_KEY; do
  [ -n "${!key:-}" ] || { echo "ERROR: $ENV_FILE 에 $key 없음/빈값 — 실패를 조용히 넘기지 않는다" >&2; exit 1; }
done
OPENAI_INFERENCEAI_EMBEDDING_ENABLED="${OPENAI_INFERENCEAI_EMBEDDING_ENABLED:-true}"
...
render_secret_manifest search-secrets "$NS" Opaque \
  OPENAI_API_KEY OPENAI_INFERENCEAI_BASE_URL OPENAI_INFERENCEAI_API_KEY \
  OPENAI_INFERENCEAI_EMBEDDING_ENABLED \
  | "$KUBECTL" apply -f -
