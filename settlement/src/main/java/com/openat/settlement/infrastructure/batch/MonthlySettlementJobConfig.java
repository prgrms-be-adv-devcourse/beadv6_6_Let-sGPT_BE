 ...
 *
 * <p>별도 설정 클래스 파일을 사용하지 않고 @Value("${settlement.batch.grid-size}") 방식으로 gridSize를 직접 주입받습니다.
 */
@Configuration
...
public class MonthlySettlementJobConfig {
  ...
  private final ReconciliationGateTasklet reconciliationGateTasklet;
   ...
   *
   * <p>application.yml: settlement.batch.grid-size
   *
   * <p>기본값을 코드에 두지 않고 application.yml 값을 반드시 사용합니다.
   */
  ...
  @Bean
  public Step reconciliationGateStep(
      JobRepository jobRepository, PlatformTransactionManager transactionManager) {
    return new StepBuilder("reconciliationGateStep", jobRepository)
        .tasklet(reconciliationGateTasklet, transactionManager)
        .build();
  }

  /** Master Step: sellerId 목록을 partition으로 나누고 Worker Step을 병렬 실행합니다. */
  @Bean
  ...
  /** Worker Step: partition 하나에 들어온 sellerId 목록을 실제 처리합니다. */
  @Bean
