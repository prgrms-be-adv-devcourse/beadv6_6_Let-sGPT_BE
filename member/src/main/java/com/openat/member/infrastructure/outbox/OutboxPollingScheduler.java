 ...
 * 잡지 않는다. 이전 구조(건별 {@code @Transactional(REQUIRES_NEW)} 안에서 {@code .get()}으로
 * ack를 기다림)는 배치 전체를 한 트랜잭션으로 묶는 문제는 없앴지만, 여전히 이벤트 1건당 최대
 * 5초는 그 트랜잭션의 커넥션을 쥐고 있었다 — payment 모듈이 이 구조(payment
 * {@code fix/payment-connection-hold})로 먼저 고쳤고, 그 패턴을 그대로 이식한 것이 이 클래스다.
 *
 ...
 * 막지 않는다.
 *
 * <p>주의(알려진 잔여 갭): 조회~발행 사이에 원자적 claim(예: 조건부 UPDATE로 PENDING→PROCESSING
 * 선점)이 없다. member가 지금은 {@code replicas: 1}(HPA 없음)이라 다중 인스턴스 동시 조회
 * 자체가 불가능해 당장은 영향이 없지만, 수평 확장 전에는 반드시 원자적 claim을 추가해야 한다.
 */
@Slf4j
@Component
public class OutboxPollingScheduler {
    ...
    public OutboxPollingScheduler(OutboxEventJpaRepository outboxEventJpaRepository,
        ...
        this.publishedCounter = meterRegistry.counter("member.outbox.published");
        Gauge.builder("member.outbox.pending", outboxEventJpaRepository,
